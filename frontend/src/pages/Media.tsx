...
export function Media() {
  ...
  // Title change handler with TMDB search
  const handleTitleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value;
    setTitle(val);
    setSelectedTmdbId(null); // Reset sync connection if they start typing again
    
    if (mediaType === 'manga' || val.trim().length < 2) {
      setTmdbResults([]);
      return;
    }
            ...
            <div className="space-y-1.5 relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm font-bold focus:ring-1 focus:ring-purple-500/50 outline-none transition-all placeholder:text-white/10 pr-10"
                />
                {isSearchingTmdb && (
